...
class InviteService {
  ...
  Future<bool> sendInviteEmail({
    ...
    print('📧 [InviteService] sendInviteEmail() chiamato');
    print('📧 [InviteService] - sourceType: ${invite.sourceType}');
    print('📧 [InviteService] - sourceName: ${invite.sourceName}');
    print('📧 [InviteService] - destinatario: ${invite.email}');

    // Use provided baseUrl or default to app URL
    final effectiveBaseUrl = baseUrl ?? 'https://pm-agile-tools-app.web.app';

    try {
      final inviteLink = generateInviteLink(invite, baseUrl: effectiveBaseUrl);
      final expirationDate = _formatDate(invite.expiresAt);
      final sourceTypeLabel = _getSourceTypeLabel(invite.sourceType);
      ...
      final safeSourceName = Validators.sanitizeHtml(invite.sourceName);

      print('📧 [InviteService] Link invito: $inviteLink');

      // Costruisci il contenuto HTML dell'email
      final htmlBody = _buildEmailHtml(
      ...
      final subject = _getEmailSubject(invite.sourceType, safeSourceName);
      final emailContent = '''From: $senderEmail
To: ${invite.email}
Subject: =?UTF-8?B?${base64Encode(utf8.encode(subject))}?=
MIME-Version: 1.0
...
''';
      ...
      try {
        ...
        print('📧 [InviteService] ✅ Email inviata con successo!');
        print('📧 [InviteService] Message ID: ${sentMessage.id}');
        return true;
      } catch (gmailError) {
        print('❌ [InviteService] ERRORE Gmail API: $gmailError');
        return false;
      }
    } catch (e, stack) {
      print('❌ [InviteService] Errore generico invio email: $e');
      print('❌ [InviteService] Stack: $stack');
      return false;
    }
  }
  ...
  String _getSourceTypeLabel(InviteSourceType type) {
    switch (type) {
      case InviteSourceType.eisenhower:
      ...
      case InviteSourceType.estimationRoom:
        return 'Estimation Room';
      case InviteSourceType.agileProject:
        return 'Progetto Agile';
      case InviteSourceType.smartTodo:
        return 'Smart Todo List';
      case InviteSourceType.retroBoard:
        return 'Retrospettiva';
    }
  }

  String _getEmailSubject(InviteSourceType type, String sourceName) {
    switch (type) {
      case InviteSourceType.eisenhower:
        return '📊 Invito Matrice Eisenhower: $sourceName';
      case InviteSourceType.estimationRoom:
        return '🎯 Invito Estimation Room: $sourceName';
      case InviteSourceType.agileProject:
        return '📋 Invito Progetto Agile: $sourceName';
      case InviteSourceType.smartTodo:
        return '✅ Invito Smart Todo: $sourceName';
      case InviteSourceType.retroBoard:
        return '🔄 Invito Retrospettiva: $sourceName';
    }
  }

  String _getSourceIcon(InviteSourceType type) {
    switch (type) {
      case InviteSourceType.eisenhower:
        return '📊';
      case InviteSourceType.estimationRoom:
  ...
  String _getHeaderColor(InviteSourceType type) {
    switch (type) {
      case InviteSourceType.eisenhower:
        return '#2196F3';
      case InviteSourceType.estimationRoom:
...
<body>
  <div class="container">
    <div class="header">
      <h1>Invito $sourceTypeLabel</h1>
    </div>
    <div class="content">
      <p>Ciao!</p>
      <p><strong>$inviterName</strong> ti ha invitato a collaborare su <strong>$sourceTypeLabel</strong>.</p>

      <div class="info-box">
        <p><strong>📋 Nome:</strong> $sourceName</p>
        <p><strong>👤 Ruolo:</strong> <span class="role-badge">$role</span></p>
