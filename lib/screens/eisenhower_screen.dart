...
import '../widgets/eisenhower/csv_import_dialog.dart';
...
class _EisenhowerScreenState extends State<EisenhowerScreen> with WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  /// Importa attività da file CSV
  Future<void> _showImportCsvDialog() async {
    final l10n = AppLocalizations.of(context)!;
    if (_selectedMatrix == null || _currentUserEmail == null) return;

    // Show import dialog
    final selectedActivities = await CsvImportDialog.show(
      context,
      matrixId: _selectedMatrix!.id,
      currentUserEmail: _currentUserEmail!,
    );

    if (selectedActivities == null || selectedActivities.isEmpty) return;

    // Create activities from CSV rows
    try {
      int importedCount = 0;
      final escapedEmail = EisenhowerParticipantModel.escapeEmail(_currentUserEmail!);

      for (final csvRow in selectedActivities) {
        // Create vote from CSV averages
        final vote = EisenhowerVote(
          urgency: csvRow.urgencyInt,
          importance: csvRow.importanceInt,
        );
        ...
        final activity = EisenhowerActivityModel(
          ...
          votes: {escapedEmail: vote},
          isRevealed: true, // Mark as revealed so it appears in quadrant
          revealedAt: DateTime.now(),
          readyVoters: [escapedEmail],
        );

        await _firestoreService.createActivityFromModel(activity);
        importedCount++;
      }

      if (mounted) {
        _showSuccess(l10n.eisenhowerImportSuccess(importedCount));
      }
    } catch (e) {
      _showError('Errore durante l\'import: $e');
    }
  }
